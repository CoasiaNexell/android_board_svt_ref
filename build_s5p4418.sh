...
if [ "${BUILD_ALL}" == "true" ] || [ "${BUILD_BL1}" == "true" ]; then
	build_bl1_s5p4418 ${BL1_DIR}/bl1-${TARGET_SOC} s5p4418 svt ${DEV_PORTNUM}
fi
...
if [ "${BUILD_ALL}" == "true" ] || [ "${BUILD_UBOOT}" == "true" ]; then
	...
	gen_third ${TARGET_SOC} ${UBOOT_DIR}/u-boot.bin \
		0x74c00000 0x74c00000 ${TOP}/device/nexell/secure/bootloader.img
fi
...
if [ -f ${UBOOT_DIR}/u-boot.bin ]; then
	UBOOT_BOOTCMD=$(make_uboot_bootcmd \
		...
		${OUT_DIR}/ramdisk.img \
		"boot:emmc")
#	UBOOT_BOOTCMD="aboot load_mmc 2580 71008000 79000000;dtimg load_mmc 23800 7A000000 0;bootz 71008000 0x79000000:\$\{ramdisk_size\} 0x7A000000"

	UBOOT_RECOVERYCMD="ext4load mmc 0:6 0x7A000000 recovery.dtb; ext4load mmc 0:6 0x71008000 recovery.kernel; ext4load mmc 0:6 0x79000000 ramdisk-recovery.img; bootz 71008000 0x79000000:27e800 0x7A000000"

	UBOOT_BOOTARGS="console=ttyAMA0,115200n8 loglevel=7 printk.time=1 androidboot.hardware=svt_ref androidboot.console=ttyAMA0 androidboot.serialno=0123456789ABCDEF quiet"
