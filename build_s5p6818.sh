...
OPTEE_BUILD_OPT="PLAT_DRAM_SIZE=1024 PLAT_UART_BASE=0xc00a1000 SECURE_ON=0 SUPPORT_ANDROID=1"
...
DTB_IMG=${KERNEL_DIR}/arch/arm64/boot/dts/nexell/s5p6818-svt-ref-rev00.dtb
...
function post_process_secure()
{
	...
	if [ "${AES_KEY}" != "none" ]; then
		...
		make_2ndboot_enc ${result_dir} ${aes_key} bl1-svt.bin bl1-usbboot.img
...
if [ "${BUILD_ALL}" == "true" ] || [ "${BUILD_BL1}" == "true" ]; then
	build_bl1 ${BL1_DIR}/bl1-${TARGET_SOC} svt 0 emmc
	build_bl1 ${BL1_DIR}/bl1-${TARGET_SOC} svt 2 sd no
fi
...
if [ "${BUILD_ALL}" == "true" ] || [ "${BUILD_KERNEL}" == "true" ]; then
	build_kernel ${KERNEL_DIR} ${TARGET_SOC} ${BOARD_NAME} s5p6818_svt_ref_nougat_defconfig ${CROSS_COMPILE}
	test -d ${OUT_DIR} && \
...
if [ -f ${UBOOT_DIR}/u-boot.bin ]; then
	...
	UBOOT_RECOVERYCMD="ext4load mmc 0:7 0x7A000000 recovery.dtb; ext4load mmc 0:7 0x71080000 recovery.kernel; ext4load mmc 0:7 0x79000000 ramdisk-recovery.img; booti 71080000 0x79000000:2d0f8f 0x7A000000"

	UBOOT_BOOTARGS="console=ttySAC0,115200n8 loglevel=7 printk.time=1 androidboot.hardware=svt_ref androidboot.console=ttySAC0 androidboot.serialno=0123456789abcdef quiet"
